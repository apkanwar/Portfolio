...
const MainCard = ({ image, title, description, tags = [], liveLink, repoLink }) => {
    return (
        <div className="max-w-sm sm:max-w-md sm:min-w-[28rem] group relative overflow-hidden rounded-xl shadow-lg bg-[#323232] z-10 mx-auto">
            ...
            {/* Bottom Section */}
            <div className="bg-[#323232] p-5 text-white flex flex-col gap-2">
                <h2 className="text-2xl font-bold font-headings">{title}</h2>
                <p className="font-dText min-h-[96px]">{description}</p>

                {/* Tags */}
                {tags.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-2 text-sm font-headings">
                        {tags.map((tag, i) => (
                ...
                <div className="mt-4 flex gap-3 items-center">
                  ...
                  {!liveLink && !repoLink && (
                    <p className="text-sm text-dm-black italic py-2 bg-white px-6 rounded-lg font-semibold">No Demo Available <span className="text-base ml-2 not-italic">😔</span></p>
                  )}
                </div>
            </div>
        </div>
