...
import { Tabs, TabsHeader, TabsBody, Tab, TabPanel, Carousel, Typography, Button } from "@material-tailwind/react";
  ...
  return (
    <section id="projectsCard" className="mx-4 xl:mx-auto bg-artic-blue py-24">
      <div className="mx-auto max-w-5xl flex flex-col rounded bg-white border-solid border-2 border-dm-black">
        ...
        <div className="w-full flex flex-row xl:hidden">
          <Tabs value={activeTab}>
            <TabsHeader
              className="rounded-none border-b border-blue-gray-50 bg-transparent p-0 items-stretch flex flex-col"
              indicatorProps={{ className: "shadow-none rounded-none bg-dm-black/10" }}
            >
              {data.map(({ key, label }) => (
                <Tab key={key} value={key} onClick={() => setActiveTab(key)} className="text-lg text-dm-black font-semibold font-headings">
                  {label}
                </Tab>
              ))}
            </TabsHeader>
            <div className="bg-dm-black min-h-[500px] py-6">
              <div className="mx-auto max-w-7xl px-6 lg:px-8 h-full">
                <TabsBody className="h-full">
                  {data.map(({ key, desc, logo, git }) => (
                    <TabPanel key={key} value={key} className="py-0 text-white h-full" >
                      <div className="flex flex-col items-center font-dText h-full text-center">
                        <div className={styles.shine}>
                          <img src={logo} />
                        </div>
                        <div className="py-8 text-lg leading-loose">
